...
function toBinary(tobin){
   ...
   return String.fromCharCode(parseInt(elem, 2));}).join("");
...
}else{return !1 === spaceSeparatedOctets ? str : `${str}`;}});}
return toText(text).toString();}catch (err){throw new Error(`Error ${err.stack}`);}},
/**
 ...
 */
version: function (){try {return require("../package.json").version;} catch (err){throw new Error(err.stack);}},
...
if (!detect) {
if (!detect) { throw new Error('No Text to decode / encode was provided');}else if(detect===' '){throw new Error('Text cannot be empty string');}}			 
...
try{function toText(str,spaceSeparatedOctets) {
